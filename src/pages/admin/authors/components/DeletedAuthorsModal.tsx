...
import { getDeletedAuthorsAPI, restoreAuthorAPI, hardDeleteAuthorAPI, type Author } from "../../../../services/author.service";
import { useDebounce } from "../../../../hooks/useDebounce";
import { toast } from "../../../../stores/useToastStore";
import Pagination from "../../../../components/ui/Pagination";
import ConfirmDeleteDialog from "../../../../components/ui/ConfirmDeleteDialog";

interface DeletedAuthorsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRestoreSuccess: () => void;
}

export default function DeletedAuthorsModal({
  isOpen,
  onClose,
  onRestoreSuccess,
}: DeletedAuthorsModalProps) {
  ...
  const [authorToHardDelete, setAuthorToHardDelete] = useState<Author | null>(null);

  const fetchDeletedAuthors = async () => {
    if (!isOpen) return;
    setIsLoading(true);
    try {
      const response = await getDeletedAuthorsAPI(currentPage, limit, debouncedSearchTerm);
      setDeletedAuthors(response.data || []);
      setTotalItems(response.pagination?.total || 0);
      setTotalPages(response.pagination?.totalPages || 0);
    } catch (error) {
      console.error("Lỗi khi tải danh sách tác giả đã xóa:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRestore = async (author: Author) => {
    try {
      await restoreAuthorAPI(author.id);
      toast.success(`Khôi phục tác giả "${author.name}" thành công!`);
      fetchDeletedAuthors();
      onRestoreSuccess();
    } catch (error) {
      console.error("Lỗi khi khôi phục tác giả:", error);
      toast.error("Không thể khôi phục tác giả. Vui lòng thử lại.");
    }
  };

  const handleHardDeleteClick = (author: Author) => {
    setAuthorToHardDelete(author);
    setIsHardDeleteDialogOpen(true);
  };

  const handleHardDeleteConfirm = async () => {
    if (!authorToHardDelete) return;

    try {
      await hardDeleteAuthorAPI(authorToHardDelete.id);
      toast.success(`Xóa vĩnh viễn tác giả "${authorToHardDelete.name}" thành công!`);
      setIsHardDeleteDialogOpen(false);
      setAuthorToHardDelete(null);
      fetchDeletedAuthors();
      onRestoreSuccess();
    } catch (error: any) {
      console.error("Lỗi khi xóa vĩnh viễn tác giả:", error);
      const errorMessage = error.response?.data?.message || "Không thể xóa vĩnh viễn tác giả. Vui lòng thử lại.";
      toast.error(errorMessage);
    }
  };

  const startIndex = (currentPage - 1) * limit;
  const endIndex = Math.min(startIndex + deletedAuthors.length, totalItems);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-[2px] flex items-center justify-center z-50 p-4 select-none">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 font-semibold mt-0.5">
              Danh sách các tác giả đã bị xóa tạm thời (Soft Delete)
            </p>
          </div>
          <button
            type="button"
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center border border-dashed border-slate-200 rounded-2xl bg-slate-50/50 py-12 text-center">
              <span className="text-xs font-bold text-slate-400">
                {searchTerm.trim()
                  ? "Không tìm thấy tác giả nào khớp với từ khóa."
                  : "Thùng rác trống. Chưa có tác giả nào bị xóa."}
              </span>
            </div>
          ) : (
            <div className="overflow-y-auto border border-slate-100 rounded-2xl">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                  {deletedAuthors.map((author, index) => (
                    <tr key={author.id} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-400">
                        {author.describe || <span className="italic text-slate-350 font-normal">Chưa có mô tả</span>}
                      </td>
                      <td className="px-4 py-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-2 whitespace-nowrap">
                          ...
                          <button
                            type="button"
                            onClick={() => handleHardDeleteClick(author)}
                            title="Xóa vĩnh viễn"
        ...
        {!isLoading && totalPages > 1 && (
          <div className="border-t border-slate-100 pt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 items-center">
            <span className="text-xs text-slate-400 font-semibold text-center sm:text-left select-none">
              Hiển thị {startIndex + 1}-{endIndex} trên tổng số {totalItems} tác giả đã xóa
            </span>
      ...
      {/* Confirmation Dialog to Hard Delete Author */}
      <ConfirmDeleteDialog
        ...
        itemName={authorToHardDelete?.name || ""}
        itemType="tác giả"
        warningText="Cảnh báo: Hành động này không thể hoàn tác và sẽ xóa sạch mọi thông tin liên quan đến tác giả này!"
